...
describe('Suite - HighlightDirective 지시자에 대한 테스트',() => {
  ...
  beforeEach(() => {
    ...
    de = fixture.debugElement.query(By.css('h3:not([highlight])'));
  });

  it('spec - 하이라이트된 엘리먼트의 개숫가 2개가 맞는지',() => {
  ...
  it('spec - 하이라이트가 없는 엘리먼트인 h3에는 myProperty 프로퍼티가 정의되지 않았는지',() => {
    expect(de.properties['myProperty']).toBeUndefined();
  });
  ...
  it('spec - 하이라이트 지시자가 없는 엘리먼트 de가 실재 null인지',() => {
    const dir = de.injector.get(HighlightDirective,null);
  ...
  it('spec - 엘리먼트 de의 제공자 토큰에 하이라이트 지시자가 포함되지 않았는지',() => {
    expect(de.providerTokens).not.toContain(HighlightDirective);
  });
